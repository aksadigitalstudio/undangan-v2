...
import { canvasBackgrounds, CanvasDocument, CanvasElement, CanvasFrame, CanvasAnimation, createCanvasDocument, ornamentLibrary } from "./canvasTypes";
...
export default function CreativeCanvas({ value, onChange, groomName, brideName, fallbackPhoto }: { value?: CanvasDocument; onChange: (document: CanvasDocument) => void; groomName: string; brideName: string; fallbackPhoto?: string }) {
  ...
  function addOrnament(src: string) {
    const element: CanvasElement = { id: newId("ornament"), type: "ornament", src, x: 58, y: 5, width: 35, height: 19, rotation: 0, zIndex: canvas.elements.length + 1, opacity: 1, animation: "float" };
    commit({ ...canvas, elements: [...canvas.elements, element] }); setSelectedId(element.id);
  }
  function duplicate() {
    if (!selected) return;
  ...
  return <div className="space-y-5">
    ...
    <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><p className="text-xs font-bold uppercase tracking-[0.16em] text-slate-500">Ornament library</p><div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-3">{ornamentLibrary.map((ornament) => <button key={ornament.id} type="button" onClick={() => addOrnament(ornament.src)} className="group overflow-hidden rounded-xl border border-slate-200 bg-slate-50 text-left transition hover:border-[#19243a]"><div className="h-16 bg-[#eee8dc]"><img src={ornament.src} alt="" className="h-full w-full object-contain transition group-hover:scale-105" /></div><span className="block px-2 py-2 text-[10px] font-semibold text-slate-600">{ornament.label}</span></button>)}</div></section>
