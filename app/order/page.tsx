...
type Props = { searchParams: Promise<{ package?: string; eventType?: string; template?: string }> };
export const metadata = { title: "Start your order" };

export default async function OrderPage({ searchParams }: Props) {
  const { package: initialPackage, eventType: initialEventType, template: initialTemplateId } = await searchParams;
