...
export default async function CheckoutPage({ searchParams }: Props) {
  ...
  const next = encodeURIComponent(`/checkout?product=${product.code}`);

  if (!user) {
    return <main className="grid min-h-screen place-items-center bg-[#fcfaf7] px-6 py-12 text-center text-[#19243a]"><div className="max-w-lg"><AksaBrand className="justify-center" /><p className="mt-12 text-xs font-bold uppercase tracking-[.22em] text-[#e26257]">One protected workspace</p><h1 className="mt-4 font-serif text-5xl leading-none">Create your account before checkout.</h1><p className="mt-6 text-base leading-7 text-[#657087]">Your payment is attached to your AKSA account. Once Midtrans confirms it, one invitation workspace is unlocked only for you.</p><div className="mt-10 flex flex-col justify-center gap-3 sm:flex-row"><Link href={`/login?next=${next}`} className="inline-flex items-center justify-center rounded-full bg-[#19243a] px-6 py-3.5 text-sm font-bold text-white">Log in to continue</Link><Link href={`/signup?next=${next}`} className="inline-flex items-center justify-center rounded-full border border-[#19243a]/15 px-6 py-3.5 text-sm font-bold">Create an account</Link></div><Link href="/pricing" className="mt-7 inline-flex items-center gap-2 text-sm font-semibold text-[#657087]"><ArrowLeft size={16} /> Back to experiences</Link></div></main>;
  }
